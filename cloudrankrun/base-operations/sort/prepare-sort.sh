...
maps_per_host=0
unit_size=0
hosts=`${HADOOP_HOME}/bin/hadoop dfsadmin -report | grep -Po "Datanodes available: \d+" | grep -Po "\d+"`
...
# fix the config file
lineno=`grep -n "bytes_per_map" config-sort.xml`
lineno=${lineno%:*}
let "lineno+=1"
sed -i "$lineno s/<value>[0-9]*<\/value>/<value>$bytes_per_map<\/value>/" config-sort.xml

lineno=`grep -n "maps_per_host" config-sort.xml`
lineno=${lineno%:*}
let "lineno+=1"
sed -i "$lineno s/<value>[0-9]*<\/value>/<value>$maps_per_host<\/value>/" config-sort.xml


echo "generating rw-sort-$size$unit data"
#${HADOOP_HOME}/bin/hadoop fs -rmr /cloudrank-data/rw-sort-$size$unit
${HADOOP_HOME}/bin/hadoop jar ../../jars/${hadoop_examples_jar}  randomwriter -conf config-sort.xml  /cloudrank-data/rtw-sort-$size$unit
sed -i "/$size$unit/d" ./file.include
sed -i "/$size$unit/d" ../../configuration/file_all.include
echo "sort_file=rtw-sort-$size$unit-$1"     >>./file.include
echo "sort_file=rtw-sort-$size$unit-$1"     >>../../configuration/file_all.include
