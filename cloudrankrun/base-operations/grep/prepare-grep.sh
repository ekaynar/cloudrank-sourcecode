...
# fix the config file
lineno=`grep -n "bytes_per_map" config-grep.xml`
lineno=${lineno%:*}
let "lineno+=1"
sed -i "$lineno s/<value>[0-9]*<\/value>/<value>$bytes_per_map<\/value>/" config-grep.xml

lineno=`grep -n "maps_per_host" config-grep.xml`
lineno=${lineno%:*}
let "lineno+=1"
sed -i "$lineno s/<value>[0-9]*<\/value>/<value>$maps_per_host<\/value>/" config-grep.xml


echo "generating rtw-grep-$size$unit data"
#${HADOOP_HOME}/bin/hadoop fs -rmr /cloudrank-data/rtw-grep-$size$unit
${HADOOP_HOME}/bin/hadoop jar ../../jars/${hadoop_examples_jar}  randomtextwriter -conf config-grep.xml  /cloudrank-data/rtw-grep-$size$unit

sed -i "/$size$unit/d" ./file.include
sed -i "/$size$unit/d" ../../configuration/file_all.include
echo "grep_file=rtw-grep-$size$unit-$1"     >>./file.include
echo "grep_file=rtw-grep-$size$unit-$1"     >>../../configuration/file_all.include
